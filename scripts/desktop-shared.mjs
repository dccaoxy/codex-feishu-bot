...
if(command==='setup'){
  ...
  const args=[binary,'-c','features.code_mode_host=true','app-server','--listen',url];
  const p=`<?xml version="1.0"?><plist version="1.0"><dict><key>Label</key><string>${label}</string><key>ProgramArguments</key><array>${args.map(a=>`<string>${xml(a)}</string>`).join('')}</array><key>WorkingDirectory</key><string>${xml(root)}</string><key>EnvironmentVariables</key><dict><key>HOME</key><string>${xml(os.homedir())}</string><key>PATH</key><string>${xml(process.env.PATH)}</string></dict><key>RunAtLoad</key><true/><key>KeepAlive</key><true/><key>ThrottleInterval</key><integer>10</integer><key>Umask</key><integer>63</integer><key>StandardOutPath</key><string>${xml(path.join(dir,'server.log'))}</string><key>StandardErrorPath</key><string>${xml(path.join(dir,'server.error.log'))}</string></dict></plist>`;
  ...
  const child=spawn(env.desktop,[],{detached:true,stdio:['ignore',log,log],env:{...process.env,CODEX_APP_SERVER_WS_URL:env.url,CODEX_APP_SERVER_FORCE_CLI:'0'}});child.unref();fs.closeSync(log);
...
}else throw new Error('用法：desktop-shared.mjs setup|status|launch-desktop|rollback|stop-server');
