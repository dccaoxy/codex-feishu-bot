...
test('retention retains topic but marks original provenance unavailable',async t=>{
 const x=fixture(t);x.raw.ingest(event('m1'),false);await x.scheduler.tick();const id=x.raw.knowledge.list('oc_A')[0].topic_id;x.raw.retentionDays=1;x.raw.prune();const result=x.raw.knowledge.read('oc_A',id);assert.equal(result.provenance.status,'partial_unavailable');assert.deepEqual(result.provenance.unavailable_source_message_ids,['m1']);
});
