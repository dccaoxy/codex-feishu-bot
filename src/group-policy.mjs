import {knowledgeConfig} from './knowledge-schema.mjs';
...
export function groupConfig(value = {}) {
  ...
  c.knowledge=knowledgeConfig(c.knowledge);
  return c;
}
export class GroupPolicy {
  ...
  mayUseTool(name, sender, chat, documentId) {
    ...
    if (this.config.knowledge.enabled && ['group_topics','group_topic_read','group_daily_digest'].includes(name)) return true;
