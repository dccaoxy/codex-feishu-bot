...
- **真实飞书已验证的部分**：机器人身份、指定测试群元数据、消息列表读取接口成功；随后用户真实群验收及本地记录补齐了未 @ 接收与 @ 回复链路的证据。用户已授权指定群及测试包启用。原飞书长连接已切换至保留 PR #5 的本地隔离候选包，77 项组合测试及共享连接 doctor、群隔离探针通过，重新启动后长连接 ready。Desktop/共享 App Server 未重启，原 checkout 和配置未覆盖。
...
- **后续实测观察**：收到一次单独 @，消息时间比候选包最后一次重启早约7秒，任务随服务停止中断且未自动重放；不能作为模型端到端成功。随后独立合成模型调用、指定群授权测试回复API均成功。已固定候选包。租户scope接口列出群全量读取及发送权限。
- **真实群基础E2E通过（15:19，北京时间）**：用户截图显示先发送未 @ 的库存资料、再精确 @ 请求整理；机器人返回正确库存、合计、来源消息和时间。只读群库核对为 recorded=1、done=1，另有此前重启中断的 failed=1，未重放。普通消息只记录，@请求完成；群消息已实际经长连接接收并被模型用于回复。本次使用近期历史，没有单独证明模型按关键词/时间调用检索工具。Markdown表格以文字原文显示，未验收为原生表格渲染。不把真实ID、群正文或截图提交到公开仓库。
- **未完成/待扩展实测**：真实群文档创建/分享、普通成员权限、撤回/退出事件仍需分别验证；时间/关键词/跨群/权限边界已有自动测试，不能冒充真实多人验收。长时间稳定性未验证。本版编辑仅首次快照，附件不解析，忙时 @ 不排队/不回话，文档写入要求明确命令；见 README。
- **提交/PR交付断点**：实现已提交并推送（`1c2ac58`），已创建独立 [PR #7](https://github.com/dccaoxy/codex-feishu-bot/pull/7)，Issue #6 已留阶段报告；基础群E2E已补齐，依 AGENTS.md 将 Draft 转 Ready 触发自动审核，尚未取得自动审核结论。Issue 保持open，不声称全部扩展实测已完成；不得自动 Merge 或进入 Full。
